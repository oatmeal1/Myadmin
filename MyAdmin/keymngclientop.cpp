...
int MngClient_InitInfo(MngClient_Info *pCltInfo)
{
	int 			ret = 0;
	//读配置文件
	KeyMng_Log(__FILE__, __LINE__,KeyMngLevel[1], 0,"func main() MngClient_InitInfo() begin");
	
	strcpy(pCltInfo->clientId, "1111");
	strcpy(pCltInfo->AuthCode, "1111");
	strcpy(pCltInfo->serverId, "0001");
	strcpy(pCltInfo->serverip, "192.168.41.253");
	...
	if (ret != 0)
	{
		KeyMng_Log(__FILE__, __LINE__,KeyMngLevel[4], ret,"func main() KeyMng_ShmInit() err");
		return ret;
	}
	
	KeyMng_Log(__FILE__, __LINE__,KeyMngLevel[1], 0,"func main() MngClient_InitInfo() end");
...
int MngClient_Check(MngClient_Info *pCltInfo)
{
	...
	if (ret != 0)
	{
		AfxMessageBox("func KeyMng_ShmRead() err");
		return ret;
	}
	//msgKeyReq.r1=前n个
	//for (i = 0; i < 8; i++)
	//{
	//	msgKeyReq.r1[i] = pNodeInfo.seckey[i];
	//}
	memcpy(msgKeyReq.r1, pNodeInfo.seckey, 8);
	...
	if (pMsgKeyRes->rv == 0)
	{
		AfxMessageBox("密钥校验 ok");
		printf("客户端与服务端密钥差球不多 大兄弟可以继续使用\n");
	}
	else
	{
		AfxMessageBox("密钥校验 err");
		printf("客户端与服务端密钥差球太多 err:%d\n", pMsgKeyRes->rv);
	}
