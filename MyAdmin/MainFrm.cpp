...
#include "MainFrm.h"
#include "FormBackground.h"  // CFormBackground  FormBackground.h
#include "CfgView.h"
#include "DlgNetInfo.h"
//#include "secmng_globvar.h"
#include "ViewClient.h"

extern CString	g_dbSource;
...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext)
{
	...
	wndBar.AddFolder("SecMngClient管理端", 1);
	...
	wndBar.InsertItem(1, 0, "SecMngClient", 6, 0);
...
//C++多态	
//多态的定义			一个api函数在家族中穿梭的时候,能表现出不同的形态; 同一个调用语句 传入子类对象 能执行子类的api函数
//多态成立的三个条件	要有继承 要有虚函数重写 父类引用(指针)指向子类对象

//C++的好处和意义
//从语法层次上 做到了支持

//封装 突破C函数的概念 给我们程序编程带了好处
//继承  可以复用前人写的代码
//多态  可以使用后来人写的代码
//(运算符重载 )
//
...
//指针重要  指针做函数参数   回调函数
//间接赋值
int getCont( char **pcont, int *len)
{
	//*pcont = 1000;
	return 0;
...
//搭建了一个舞台 让对象唱戏
void HowToPrint(MyParent *base)  //15:48
{
	base->print(); //能表现出多种形态 //支持
	base->age;
}
...
void mainxxx()
{
	MyParent	p1;  //提前布局 vptr 
	MyChild		c1;   //C++编译器为函数虚函数的类的对象 提前布局了一个vptr指针和虚函数表
